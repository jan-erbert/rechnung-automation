...
def lade_konfiguration(pfad: Path = BASE_DIR / "data" / "konfiguration.json") -> dict:
    """Lädt und prüft die zentrale Rechnungs-Konfiguration."""
...
def lade_verlauf_datei(dateiname, jahr):
    """Lädt eine Verlaufsdatei oder legt sie bei Bedarf neu an."""
...
def berechne_stundenleistung(firma: str, zyklus: int, stundensatz: float):
    """Berechnet stundenbasierte Leistungen für den Abrechnungszeitraum."""
...
def main():
    ...
    rechnungsverlauf_vorjahr = (
        lade_verlauf_datei(vorjahr_dateiname, jahr - 1)
