...
from tools import migrate_legacy_data
...
def schreibe_json_quellen(tmp_path):
    ...
    data_json.write_text(json.dumps(customers), encoding="utf-8")
...
def migriere_testdaten(tmp_path, monkeypatch):
    ...
    assert migrate_legacy_data.main() == 0
...
def test_migration_writes_yaml_without_changing_json_sources(
    ...
    assert load_customer_files(customers_dir)[0]["id"] == "musterfirma"
    assert load_invoice_config(invoice_yaml)["sender"]["name"] == "Max Mustermann"
...
def test_verify_rejects_changed_customer_yaml(tmp_path, monkeypatch):
    ...
    customer["contact"]["company"] = "Veraenderte Firma"
